...
fn build_document_object(
    ...
    let doc_for_get: &'static Document<'static> = unsafe { &*(document as *const Document<'static>) };
    let descendants = doc_for_get.descendants();
    let get_element_by_id_fn = unsafe { NativeFunction::from_closure(move |_this, args, ctx| {
        let id = args
            .first()
    ...
    let doc_for_qs: &'static Document<'static> = unsafe { &*(document as *const Document<'static>) };
    let descendants_qs = doc_for_qs.descendants();
    let query_selector_fn = unsafe { NativeFunction::from_closure(move |_this, args, ctx| {
        let selector = args
            .first()
            .map(|v| v.to_string(ctx).map(|s| s.to_std_string_escaped()))
            .unwrap_or(Ok(String::new()))?;
                ...
                node.as_element().is_some_and(|el| {
                    ...
                    selector.eq_ignore_ascii_case(tag)
                        || selector.strip_prefix('.').is_some_and(|class| {
                            el.attr("class")
                                .is_some_and(|c| c.split_whitespace().any(|p| p.eq_ignore_ascii_case(class)))
                        })
                        || selector.strip_prefix('#').is_some_and(|id| {
                            el.attr("id").is_some_and(|attr| attr.eq_ignore_ascii_case(id))
                        })
                })
            });
    ...
    let create_element_fn = unsafe { NativeFunction::from_closure(|_this, args, ctx| {
        let tag = args
            .first()
            .map(|v| v.to_string(ctx).map(|s| s.to_std_string_escaped()))
            .unwrap_or(Ok(String::new()))?;
        ...
        if tag.is_empty() {
            return Err(BoaError::from(JsNativeError::typ().with_message("createElement: tag name is required")));
        }

        let el = ObjectInitializer::new(ctx)
            .property(js_string!("tagName"), JsString::from(tag.to_uppercase()), Attribute::all())
...
fn build_window_object(context: &mut Context) -> JsResult<JsValue> {
    ...
    let alert_fn = unsafe { NativeFunction::from_closure(|_this, args, ctx| {
        let msg = args
            .first()
            .map(|v| v.to_string(ctx).map(|s| s.to_std_string_escaped()))
            .unwrap_or(Ok(String::new()))?;
    ...
    obj.set(js_string!("alert"), FunctionObjectBuilder::new(context.realm(), alert_fn).build(), false, context)
