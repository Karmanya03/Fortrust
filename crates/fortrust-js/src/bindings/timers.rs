...
use boa_engine::{
    Context, JsResult, JsValue, NativeFunction, js_string, object::FunctionObjectBuilder,
};
use tracing::debug;
...
pub fn register(
    ...
    let set_timeout_val: JsValue = FunctionObjectBuilder::new(context.realm(), set_timeout_fn)
        .build()
        .into();
    let set_interval_val: JsValue = FunctionObjectBuilder::new(context.realm(), set_interval_fn)
        .build()
        .into();
    let clear_timeout_val: JsValue = FunctionObjectBuilder::new(context.realm(), clear_timeout_fn)
        .build()
        .into();
    let clear_interval_val: JsValue =
        FunctionObjectBuilder::new(context.realm(), clear_interval_fn)
            .build()
            .into();
